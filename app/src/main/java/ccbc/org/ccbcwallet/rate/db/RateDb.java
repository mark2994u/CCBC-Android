package ccbc.org.ccbcwallet.rate.db;
...
import ccbc.org.ccbcwallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<CcbcRate> implements RateDbDao<CcbcRate> {
    ...
    @Override
    protected ContentValues buildContent(CcbcRate obj) {
    ...
    @Override
    protected CcbcRate buildFrom(Cursor cursor) {
        ...
        return new CcbcRate(coin,value,timestap);
    }

    public CcbcRate getRate(String coin){
        return get(KEY_COIN,coin);
    }


    public void insertOrUpdateIfExist(CcbcRate ccbcRate) {
        if (getRate(ccbcRate.getCode())==null){
            insert(ccbcRate);
        }else {
            updateByKey(KEY_COIN,ccbcRate.getCode(),ccbcRate);
        }
    }
}
