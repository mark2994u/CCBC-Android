package ccbc.org.ccbcwallet.ui.pincode_activity;
...
import ccbc.org.ccbcwallet.ui.backup_mnemonic_activity.MnemonicActivity;
import ccbc.org.ccbcwallet.ui.base.BaseActivity;
import ccbc.org.ccbcwallet.ui.settings_pincode_activity.KeyboardFragment;
import ccbc.org.ccbcwallet.ui.start_activity.StartActivity;

import static ccbc.org.ccbcwallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (ccbcApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (ccbcApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            ccbcApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        ccbcApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(ccbcApplication.getAppConf().getPincode().equals(pincode)){
