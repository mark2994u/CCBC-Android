...
import ccbc.org.ccbcwallet.ui.contacts_activity.ContactsActivity;
import ccbc.org.ccbcwallet.ui.donate.DonateActivity;
import ccbc.org.ccbcwallet.ui.settings_activity.SettingsActivity;
import ccbc.org.ccbcwallet.ui.wallet_activity.WalletActivity;

import static ccbc.org.ccbcwallet.module.CcbcContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends CcbcActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = ccbcApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = ccbcModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (ccbcModule.getChainHeight()+10>peerHeight) {
            ...
            if (ccbcModule.isAnyPeerConnected()) {
                long peerHeight = ccbcModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (ccbcModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = ccbcModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (ccbcModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
