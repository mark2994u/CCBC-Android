...
import ccbc.org.ccbcwallet.ui.base.BaseRecyclerFragment;
...
import ccbc.org.ccbcwallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static ccbc.org.ccbcwallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static ccbc.org.ccbcwallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static ccbc.org.ccbcwallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = ccbcModule.listTx();
    ...
    protected BaseRecyclerAdapter<TransactionWrapper, ? extends BaseRecyclerViewHolder> initAdapter() {
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (ccbcRate!=null) {
                    localCurrency = ccbcApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * ccbcRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                                    + " " + ccbcRate.getCode();
                ...
                holder.title.setText(getAddressOrContact(ccbcModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(ccbcModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(ccbcModule.getConf().getNetworkParams()).toBase58());
                }*/
